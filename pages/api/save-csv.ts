...
export default async function handler(
  ...
  if (req.method !== "POST") {
    return res.status(405).json({ message: "Method Not Allowed TEST" });
  }
  ...
  // TEST
  const testScoresData: CsvData[] =
    moduleType !== "Demographic" &&
    testScores.map((answer: any, index: number) => {
      ...
      const personalityScore = getQuestionAnswerPersonalityScore(
        index + 1,
        parsedAnswer
      );
      const abilityScore = getQuestionAnswerAbilityScore(
        index + 1,
        parsedAnswer
      );
      const inferentialScore = getQuestionAnswerInferentialScore(
        index + 1,
        parsedAnswer
      );
  ...
  const demoAnswerData: {
    ...
    moduleType === "Demographic" && testScores
      ? Object.keys(testScores).map((questionNumber, index) => {
          const demoAnswer =
            Number(questionNumber) === 7 ||
            Number(questionNumber) === 8 ||
            Number(questionNumber) === 15
          ...
          const time =
            timeRecords[`Question ${Number(questionNumber)}`] || "N/A";
  ...
  let csvContent =
    moduleType === "Demographic"
      ? generateCsvContentForDemographic(demoAnswerData)
      : generateCsvContent(testScoresData, moduleType, timeRecords);
...
function generateCsvContentForDemographic(demoAnswerData: any[]): string {
  ...
  const rows = demoAnswerData
    .map(({ sNo, question, answer, specification, time }) => {
      const answerText = typeof answer === "object" ? answer.answer : answer;
      const specText =
        typeof answer === "object" ? answer.specification : specification;
