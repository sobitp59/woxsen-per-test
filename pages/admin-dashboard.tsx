...
import { Flex, Text, Table, Thead, Tbody, Tr, Th, Td, Button, Box, useColorModeValue } from '@chakra-ui/react';
...
export default function AdminDashboard() {
  ...
  const [excelFileNames, setExcelFileNames] = useState<{ [key: string]: string }>({});
  ...
  async function createExcelFiles(files: string[]) {
    ...
    files.forEach(file => {
      ...
      if (match) {
        ...
        if (file.includes('demographic')) groupedFiles[incrementNumber].demographic.push(file);
        else if (file.includes('personality')) groupedFiles[incrementNumber].personality.push(file);
        else if (file.includes('psychometric')) groupedFiles[incrementNumber].psychometric.push(file);
      }
    });
    ...
    const excelFiles = await Promise.all(
      Object.keys(groupedFiles).map(async incrementNumber => {
        ...
        if (filesToProcess.demographic.length > 0 || filesToProcess.personality.length > 0 || filesToProcess.psychometric.length > 0) {
          try {
            ...
            console.error(`Error creating Excel file for increment ${incrementNumber}:`, error);
            return null;
          }
        }
        return null;
    ...
    const validFiles = excelFiles.filter(file => file !== null) as { incrementNumber: string, url: string, name: string }[];
    setExcelFileUrls(validFiles.reduce((acc, file) => ({ ...acc, [file.incrementNumber]: file.url }), {}));
    setExcelFileNames(validFiles.reduce((acc, file) => ({ ...acc, [file.incrementNumber]: file.name }), {}));
    setLoading(false);
  }
  ...
  return (
    <Flex direction="column" p={5} bg={useColorModeValue("gray.50", "gray.900")} minH="100vh">
     <Button
        mb={4}
        bg="red.500"
        color="white"
      ...
      {loading ? (
        ...
        Object.keys(excelFileUrls).map(incrementNumber => (
          <Flex key={incrementNumber} direction="column" align="center" mt={6} bg={tableBg} p={5} borderRadius="md" shadow="md" border={`1px solid ${tableBorderColor}`}>
            <Table variant="simple" size="md">
              <Thead>
                <Tr>
                  <Th color={customColor} fontSize="lg">Test Scores</Th>
                  <Th color={customColor} fontSize="lg">Download Excel Files</Th>
                </Tr>
              </Thead>
              <Tbody>
                <Tr>
